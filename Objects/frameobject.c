...
static void reset(codetracker *tracker) {
    tracker->offset = 0;
    tracker->addr = 0;
    tracker->line_addr = 0;
...
static void init_codetracker(codetracker *tracker, PyCodeObject *code_obj) {
...
static void advance_tracker(codetracker *tracker) {
    tracker->addr += sizeof(_Py_CODEUNIT);
    if (tracker->offset >= tracker->lnotab_len) {
        return;
    }
    int next_addr = tracker->line_addr + tracker->lnotab[tracker->offset];
...
static void retreat_tracker(codetracker *tracker) {
    tracker->addr -= sizeof(_Py_CODEUNIT);
...
static int move_to_addr(codetracker *tracker, int addr) {
...
static int first_line_not_before(codetracker *tracker, int line) {
    int result = INT_MAX;
    reset(tracker);
...
static void init_blockstack(blockstack *blocks) {
    blocks->depth = 0;
}
...
static unsigned char pop_block(blockstack *blocks) {
    assert(blocks->depth > 0);
    blocks->depth--;
    return blocks->stack[blocks->depth].kind;
}

static blockitem *top_block(blockstack *blocks) {
    assert(blocks->depth > 0);
    return &blocks->stack[blocks->depth-1];
}

static inline int is_try_except(unsigned char op, int target_op) {
    return op == SETUP_FINALLY && (target_op == DUP_TOP || target_op == POP_TOP);
}

static inline int is_async_for(unsigned char op, int target_op) {
    return op == SETUP_FINALLY && target_op == END_ASYNC_FOR;
}

static inline int is_try_finally(unsigned char op, int target_op) {
    return op == SETUP_FINALLY && !is_try_except(op, target_op) && !is_async_for(op, target_op);
}

#define TRY_EXCEPT 250

static int block_stack_for_line(codetracker *tracker, int line, blockstack *blocks) {
