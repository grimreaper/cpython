...
class LGettextTestCase(GettextBaseTest):
    ...
    def test_lpgettext(self):
        lpgettext = gettext.lpgettext
        ldpgettext = gettext.ldpgettext
        lnpgettext = gettext.lnpgettext
        ldnpgettext = gettext.ldnpgettext
        self.assertEqual(lpgettext('my context', 'nudge nudge'),
                         b'wink wink (in "my context")')
        self.assertEqual(lpgettext('my context', 'spam'), b'spam')
        self.assertEqual(ldpgettext('gettext', 'my context', 'nudge nudge'),
                         b'wink wink (in "my context")')
        self.assertEqual(ldpgettext('gettext', 'my context', 'spam'), b'spam')
        self.assertEqual(lnpgettext('With context',
                                    'There is %s file', 'There are %s files', 1),
                         b'Hay %s fichero (context)')
        self.assertEqual(lnpgettext('With context',
                                    'There is %s file', 'There are %s files', 2),
                         b'Hay %s ficheros (context)')
        self.assertEqual(ldnpgettext('gettext', 'With context',
                                     'There is %s file', 'There are %s files', 1),
                         b'Hay %s fichero (context)')
        self.assertEqual(ldnpgettext('gettext', 'With context',
                                     'There is %s file', 'There are %s files', 2),
    ...
    def test_lpgettext_2(self):
        with open(self.mofile, 'rb') as fp:
            t = gettext.GNUTranslations(fp)
        lpgettext = t.lpgettext
        self.assertEqual(lpgettext('my context', 'nudge nudge'),
                         b'wink wink (in "my context")')
        self.assertEqual(lpgettext('my context', 'spam'), b'spam')

    def test_lpgettext_bind_textdomain_codeset(self):
        lpgettext = gettext.lpgettext
        ldpgettext = gettext.ldpgettext
        saved_codeset = gettext.bind_textdomain_codeset('gettext')
        try:
            gettext.bind_textdomain_codeset('gettext', 'utf-16')
            self.assertEqual(lpgettext('my context', 'nudge nudge'),
                             'wink wink (in "my context")'.encode('utf-16'))
            self.assertEqual(lpgettext('my context', 'spam'),
                             'spam'.encode('utf-16'))
            self.assertEqual(ldpgettext('gettext', 'my context', 'nudge nudge'),
                             'wink wink (in "my context")'.encode('utf-16'))
            self.assertEqual(ldpgettext('gettext', 'my context', 'spam'),
                             'spam'.encode('utf-16'))
        finally:
            del gettext._localecodesets['gettext']
            gettext.bind_textdomain_codeset('gettext', saved_codeset)

    def test_lpgettext_output_encoding(self):
        with open(self.mofile, 'rb') as fp:
            t = gettext.GNUTranslations(fp)
        lpgettext = t.lpgettext
        t.set_output_charset('utf-16')
