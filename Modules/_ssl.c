...
static int EVP_PKEY_up_ref(EVP_PKEY *p) {
    CRYPTO_add(&p->references, 1, CRYPTO_LOCK_EVP_PKEY);
    return 1;
}
...
/* LibreSSL is missing OpenSSL API
 *
 * Not exactly the same functionality, hopefully good enough for LibreSSL.
 */
static int
...
long SSL_CTX_clear_chain_certs(SSL_CTX *ctx) {
...
/*[clinic end generated code: output=454c7e41230ca551 input=997f1fb3a784ef88]*/
{
    ...
    if (cadata) {
        ...
        else if (PyObject_GetBuffer(cadata, &buf, PyBUF_SIMPLE) == 0) {
