...
static BIO *
_PySSL_filebio(PyObject *path)
{
    BIO *bio = NULL;
    int result;

    if ((bio = BIO_new(BIO_s_file())) == NULL) {
        _setSSLError("Can't allocate BIO", 0, __FILE__, __LINE__);
        return NULL;
    }
    ...
    if (result <= 0) {
        _setSSLError("Cannot read file", 0, __FILE__, __LINE__);
        BIO_free(bio);
        return NULL;
    }
    return bio;
...
_PySSL_bufferbio(Py_buffer *b)
{
    ...
    if (bio == NULL) {
        _setSSLError("Cannot allocate BIO", 0, __FILE__, __LINE__);
        return NULL;
    }
    return bio;
}
 ...
 */
static PyObject *
_PySSL_UnicodeFromBIO(BIO *bio, const char *error)
{
    long size;
    char *data = NULL;
    size = BIO_get_mem_data(bio, &data);
